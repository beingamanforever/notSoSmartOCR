"""Benchmark the local Falcon-OCR core reader on the frozen 22-page panel."""
...
from ocr_pipeline.providers import LocalReader
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model", default=FALCON_MODEL_ID)
    parser.add_argument("--revision", default=FALCON_MODEL_REVISION)
    parser.add_argument("--category", choices=FALCON_OCR_CATEGORIES, default="plain")
    parser.add_argument("--max-new-tokens", type=_token_limit, default=3072)
    return parser
...
def run_benchmark(
    ...
    *,
    ...
    revision: str = FALCON_MODEL_REVISION,
    category: str = "plain",
    ...
    if max_new_tokens <= 0 or max_new_tokens > 3072:
        raise ValueError("max_new_tokens must be from 1 to 3072")

    cases = _select_cases(load_page_cases(challenge_root, "full", ()), case_ids)
    page_reader = reader or FalconOCRReader(
    ...
    with tempfile.TemporaryDirectory(prefix="falcon-page-benchmark-") as temporary:
    ...
    report = {
        "benchmark": "Falcon-OCR frozen clinical page challenger",
        "status": "complete",
        "cases": {
        ...
        "model": getattr(
            page_reader, "provenance", {"requested": str(model_name_or_path)}
        ),
        "generation": getattr(
            page_reader,
            "generation",
...
def _token_limit(value: str) -> int:
    parsed = int(value)
    if parsed <= 0 or parsed > 3072:
        raise argparse.ArgumentTypeError("must be from 1 to 3072")
    return parsed
