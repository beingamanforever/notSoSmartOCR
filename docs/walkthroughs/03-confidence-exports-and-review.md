...
The request lists crop IDs and, for selected tables, independently predicted row counts, column counts and merged-cell spans; source coordinates stay local for reconstruction. The model reads the labeled pixels without repeating that geometry in its prompt. Each crop uses its original detected bounds, without expanding to a whole row or page. Every crop records its source ID and routing reasons locally. Existing repetition/date validation flags and unresolved recognition can route individual regions. An isolated low decoder token score does not trigger an API request: it is uncalibrated, and in the local routing audit that policy would have sent nearly every page. Scores remain diagnostic evidence until routing quality is measured against verified errors. A missed or undersized detection still needs a detector/ownership repair; selective cropping cannot recover pixels outside its input.

The caller supplies the model and `OPENROUTER_API_KEY`; zero-data retention remains enabled by default. Any retention exception must be explicitly authorized for the data being processed. Raw and refined outputs should be saved separately, including failed requests. Crop recognition can still misread handwriting, associate a note with the wrong row, or omit content. Replacement applies by source ID in a separate rendering view, refreshing affected layout owners while keeping source evidence unchanged. Incomplete crop responses and non-renderable table responses retain the original Markdown with an invalid status. Selected tables also pass through the existing [TableFormer adapter](../../src/ocr_pipeline/tableformer_structure.py), which predicts structure from the crop independently of Qwen. Disagreement in rows, columns or spans retains the original output for review. Neither agreement nor valid markup proves that cell text is correct. TableFormer can itself miss a column; the pipeline does not trim the Qwen output to force agreement.

This optional path requires `docling-ibm-models` and downloads the pinned TableFormer weights on first use. The default CPU predictor is reused across requests; callers can supply a configured `TableFormerStructure` instance. It does not enable Docling document conversion or Table Transformer in the workbench. Use the recognition source image, not a reduced UI preview. The private PDF runner now renders the source at the saved OCR frame dimensions. Input crops, independently predicted geometry, source pixel dimensions and model provenance are saved separately.

TableFormer comes from IBM, with [MIT code](https://github.com/docling-project/docling-ibm-models/blob/main/LICENSE) and [CDLA-Permissive-2.0 weights](https://huggingface.co/ds4sd/docling-models/tree/v2.3.0), checked 9 September 2026. A bounded handwritten-table comparison improved from 16/20 source-reviewed clear cells and seven blank rows to 20/20 and the correct six blank rows using source-frame crops, TableFormer and a separate Qwen3.8 candidate. Ambiguous crossed-out content and marginal marks were excluded from that cell score. Qwen3.7 variants still showed transcription or row errors; no corpus-wide quality claim or automatic model switch follows from this case.
